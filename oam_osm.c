// Linear Covolution by Overlap Add Method
//AUTHOR  : Komal Mane
...
//initializing the various functions that are  required
...
main()
{
    ...
    float h[max];		//initialize necessary variables and arrays

    for(i=0;i<max;i++)  // Initialization of the required arrays to zero  
    {
       x[i]=0; 
       h[i]=0;
       y[i]=0;
    ...
    //Obtaining the input specifications from user

    printf("\n Enter the length of x[n] = : ");
    scanf("%d",&SigLen);                              //length of the input signal
    ...
    for(i=0;i<SigLen;i++)
    {
	 scanf("%f",&x[i]);                              //values of the long input signal stored in the respective array
    }
    ...
    for(i=0;i<M;i++)
    {
	scanf("%f",&h[i]);                              //values of the signal stored in its respective array
    }

// Procedure for OAM begins

	
    N = 8; // Assumption for value of N
    
    L = N - M; // Length of decomposed x[n]
	
    printf( "\n length of decomposed output Signal : N = %d \n ", N);
    
    j = 0;                //counter to count the decomposed parts of x[n]
    ...
    for (k=0; k<SigLen; k+=L)
    {
    	...
    	Fast_Circular_Convolution(p, N, h, t); // LC By CC by FFT
    	
    	for(i=0; i<N; i++)  
    	{
    		y[k+i] = y[k+i]+t[i]; // Overlap Adding and storing in the respective array
        }
    	
    	j++;                    // counter increament
    	
    	printf("\n\n\n x%d[n] = ", j );   // Decomposed x[n]
        for(i=0;i< N; i++)    
    	printf("  %4.2f  ",p[i]);   //printing the decomposed x(n) with 4 significant digits and 2 digits after decimal point.
	
	    printf("\n\n y%d[n] = ",j );  // Decomposed y[n]
        for(i=0;i< N;i++)    
    	printf("  %4.2f  ",t[i]);        //printing the corresponding decomposed y(n) with 4 significant digits and 2 digits after decimal point.
    ...
    for(i=0;i<(SigLen+M-1);i++)    
    	printf("  %4.2f  ",y[i]);          //printing the entire obtained y(n) 
    	
    printf("\n\n");

}

//function to calculate circular convolution using fft
...
void Fast_Circular_Convolution(float *x, int N, float *h, float *y)
{
	...
	for(k=0; k<N; k++)  // Initialization of arrays to zero
        {
        ...
        for(i=0;i<N;i++) // Copy x[n] to t[n][0], a temporary array
        {
        ...
        // To find X[k], check whether signal is 4 point or 8 point and then call the function for DITFFT accordingly
        ...
        // To Find H[k], check whether signal is 4 point or 8 point and then call the function for DITFFT accordingly
       ...
       // Find Y[k], initialize values of variables from arrays and calculate
        ...
        for(k=0; k<N; k++)  
        {
              ...
              Y[k][0] = (a * c) - (b * d);      //perform the convolution between x[n] and h[n] by multiplying X[k] and H[k]
              Y[k][1] = (b * c) + (a * d);      //hence obtaining Y[k]
              
        }
    
    
       // Find Y*[k], conjugate of previous function
       ...
       // To find FFT{Y*[k]}, check whether signal is 4 point or 8 point and then call the function for FFT accordingly
...
// to fint DITFFT of an 8 point signal 
...
void DITFFT_8_Point( int N, float x[8][2], float t[8][2])
{
     	...
     	int i,j,k,a,b,c,d;
      	float e;
        float X1[4][2], X2[4][2], G[4][2], H[4][2];		//Initialization of necessary arrays and variables
	
	 
	
	for(i = 0; i < 4; i++)              //splitting the function into N/2 long signals where N=8. Therefore N/2=4
	{
        	X1[i][0] = x[2*i][0];    
			X1[i][1] = x[2*i][1];     
            X2[i][0] = x[2*i+1][0];
	...
	//call function for 4 point DITFFT calculations

	
	 
	// X[k] = G[k] + W H[k]
           	
        e = 6.283185307179586/N;	//calculation of Twiddle Factor	
        
        for(k=0; k<4; k++)                            //for N=0,2,4,6
        {
